...
namespace CourseWork
{
    public partial class ClientRequests : Form
    {
        ...
        private void contracts_LoadData()
        {

            DB db = new DB();
            db.openConnection();
            String query;
            query = "SELECT [Renters.Name], [Renters.Surname],[Areas.AreaName],[Areas.Rooms], " +
                "[Areas.SpaceOfArea_squareMeter],[Contracts.Amount_of_money]   " +
                "FROM ((Areas INNER JOIN Contracts ON Areas.id=Contracts.Area_id) INNER JOIN Renters ON Contracts.Renter_id=Renters.id) " +
                "WHERE Contracts.Owner_id=@o_id";
